...
def page_footer(page) -> str:
    ...
    try:
        ...
        return (
            f'<div class="text-muted">Voir le <a href="{GITHUB_BASE}">code source de cette page</a>.</div>'
        )
    else:
        commit = next(repo.iter_commits(paths=blob.path, max_count=1))
        last_update_dt = datetime.datetime.fromtimestamp(commit.committed_date)
        last_update_str = last_update_dt.strftime("%A %-d %B %Y à %H:%M:%S")
        return (
            "<div>"
            f'<a href="{GITHUB_BASE}">Code source (git)</a>'
            r"</div>"
            f'<div class="text-muted">Mis à jour le {last_update_str} par '
